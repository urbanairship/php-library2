...
def php_function_role(typ, rawtext, text, lineno, inliner, options={}, content=[]):
    text = utils.unescape(text)
    env = inliner.document.settings.env
    base_url = env.app.config.api_url
    has_explicit_title, title, namespace_and_function = split_explicit_title(text)
    ...
    try:
        full_url = base_url % namespace.replace('\\', '.') + '.html' + '#function_' + function
    except (TypeError, ValueError):
        env.warn(env.docname, 'unable to expand %s api_url with base '
                 'URL %r, please make sure the base contains \'%%s\' '
                 'exactly once' % (typ, base_url))
    ...
    if not has_explicit_title:
        title = function + '()'
    list = [nodes.reference(title, title, internal=False, refuri=full_url, reftitle=namespace + '\\' + function + '()')]
    pnode = nodes.literal('', '', *list)
    return [pnode], []
...
def setup(app):
    ...
    app.add_role('function', php_function_role)
    app.add_role('constant', php_constant_role)
